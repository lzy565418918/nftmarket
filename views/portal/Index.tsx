...
import { hhnft, hhresell, hhnftcol, mainnet, cipherHH, simpleCrypto } from '@/engine/configuration'; // 合约地址和加密配置
...
export default function Sell() {
  ...
  const [created, getCreated] = useState<NftItem[]>([]); // 创建的NFT数组
  ...
  const getWalletNFTs = async () => {
    ...
    try {
      ...
      const contract = new ethers.Contract(hhnftcol, Collection, wallet); // Collection 合约实例
      ...
      for (let i = 0; i < totalSup; i++) {
        ...
        const owner = await contract.ownerOf(token).catch((error) => {
          console.log('tokens filtered:', error); // 获取持有者失败处理
        });
        const rawUri = await contract.tokenURI(token).catch((error) => {
  ...
  const getCreatedNFTs = async () => {
    ...
    try {
      ...
      console.log('Error fetching  create NFTs:', error); // 错误处理
    } finally {
      setloading(false);
    }
  }
  ...
  return (
    <div className="max-w-[900px] mx-auto my-10 p-6">
          ...
          nfts.map((nft, i) => {
            ...
            if (owner.indexOf(nft.wallet) !== -1) {
              ...
              async function relistNFT() {
                ...
                const contractnft = new ethers.Contract(hhnftcol, Collection, signer); // Collection 合约实例
                await contractnft.setApprovalForAll(hhresell, true); // 授权市场合约
                const contract = new ethers.Contract(hhresell, NFTMarketResell, signer); // NFTMarketResell 合约实例
      ...
      <p className='text-2xl mt-[16px]'>Created NFT's in Wallet</p>
