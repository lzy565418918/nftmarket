...
contract n2DMarket is ReentrancyGuard, Ownable {
    ...
    function createVaultItem(
        ...
        // 验证 nftContract 是否为 ERC721
        IERC721 testNFT = IERC721(nftContract);
        require(
            testNFT.supportsInterface(0x80ac58cd), // ERC721 接口 ID
            "Not an ERC721 contract"
        );
        require(
            testNFT.ownerOf(tokenId) == msg.sender,
            "Caller not the owner of the NFT"
        );
        _itemIds.increment();
        ...
        idToVaultItem[itemId] = VaultItem({
            ...
            holder: payable(address(0)),
            price: price,
            sold: false
        });
        testNFT.transferFrom(msg.sender, address(this), tokenId);
